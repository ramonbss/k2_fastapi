...
from app.api.crud import create_purchases, get_user_by_id, get_admin_reports_from_db
...
@users_router.get("", tags=[TAG_USERS])
async def user_purchases(user_id: int = Depends(validate_user_id)):
    ...
    users_purchases = get_admin_reports_from_db(user.id)
